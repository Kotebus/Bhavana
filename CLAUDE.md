...
The `<Stack/>` in `app/_layout.tsx` configures per-screen options (theme-driven `contentStyle` / `headerStyle` / `headerTintColor`, plus iOS-only native headers for the About sub-screens with `t(...)` titles). Transition animation is pinned: `animation: 'ios_from_right'` (iOS-style slide with parallax on both platforms; also enables left-edge swipe-back on Android) at `animationDuration: 150` (snappy — feel free to tune in `RootStack`'s `screenOptions`).

`@react-navigation/native` and `@react-navigation/native-stack` are **not** direct dependencies — `@react-navigation/native` ships transitively via `expo-router` and is sufficient for our usage; if you `npm install @react-navigation/native` directly, `expo-doctor` will flag it under SDK 56's compatibility rules.
...
Notable palette tokens beyond the obvious `background` / `text` / `border`:

- `pickerSurface` / `pickerText` — used by both the Home time picker and the Settings reciter picker so they share a brighter, distinct surface (vs the regular `surface`/`controlBg` used elsewhere). The dark-theme `pickerText` is intentionally `#EDEDED`, not pure white, because the latter felt aggressive on top of `pickerSurface`.
- `pickerBorder` — the time picker container border (Android only). Kept black in light theme on purpose (the muted `border` looked too faint there).
- `link` — link color shared by markdown and the inline links rendered through `useLinkTextStyles()` in `components/common/TextWithLink.tsx`.
- `markdownText` / `markdownHeading` — feed `<Markdown>`'s `theme.colors.text` / `theme.colors.heading`.
- The meditation timer counter intentionally uses `subtleText` (not `text`) for a calmer presence during sessions.
...
`@react-native-picker/picker` quirks worth knowing:

- iOS uses `UIPickerView` (wheel) and only honors `itemStyle` for the row text (`color`, `fontSize`, …). The wheel's center selection bar is system-styled and cannot be repainted from JS — that's why we don't try to add a custom highlight there.
- Android with `mode='dropdown'` shows a system popup whose **background** is controlled by the OS theme (light/dark), not by our props. `itemStyle` colors apply to the individual items inside, but the popup chrome itself remains light if the device OS is in light mode. The "trigger" surface (the part rendered inside our app's screen) is themed normally via `pickerSurface` / `pickerText`.

### EAS / release configuration

`eas.json` is set to `cli.appVersionSource: "remote"` with `build.production.autoIncrement: true`. That means the Android `versionCode` is **managed by EAS**, not by `android/app/build.gradle` (which is regenerated by `prebuild --clean` anyway). When bumping for a release:

- `versionName` lives in `app.json` (`expo.version`) and `package.json`. Bump both together.
- `versionCode` is auto-incremented by EAS on every production build. To realign with Play Store (e.g. after re-creating the EAS counter), use `eas build:version:get -p android` and `eas build:version:set -p android`.

### .npmrc

The project root has `.npmrc` with `legacy-peer-deps=true`. Without it, EAS Build's `npm ci` fails on the TypeScript 5 vs 6 peerOptional clash between Expo SDK 56 transitive deps and `react-i18next@^15.6.1`. Local installs use the same resolution.
